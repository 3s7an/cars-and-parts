...
<template>
    <Teleport to="body">
        <div v-if="showWidget" class="fixed bottom-5 right-5 z-[100] flex flex-col items-end gap-3">
        <div
            v-if="isOpen"
        ...
        >
            ...
            <div ref="messagesContainer" class="flex-1 space-y-3 overflow-y-auto p-4">
                ...
                >
                    Ahoj! Pomôžem ti nájsť autá alebo diely. Napríklad: „Hľadám brzdy pre Octaviu.“
                </p>
                ...
                <div v-if="isLoading" class="flex justify-start">
                    <div class="rounded-2xl bg-muted px-3 py-2 text-sm text-muted-foreground">
                        Píšem odpoveď…
                    </div>
                </div>
            </div>

            <div v-if="error" class="border-t px-4 py-2 text-sm text-destructive">
                {{ error }}
            </div>

            <form class="flex items-end gap-2 border-t p-3" @submit.prevent="handleSubmit">
                <textarea
                    v-model="draft"
                    rows="2"
                ...
                />
                <Button type="submit" size="icon" class="shrink-0 rounded-xl" :disabled="isLoading || !draft.trim()">
                    <Send class="size-4" />
                </Button>
            </form>
        </div>
