...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    { title: 'Cars', href: cars().url },
    {
        ...
        href:
            isEdit.value && props.car
                ? edit.url({ car: props.car.id })
                : create().url,
    },
]);
...
function submit() {
    ...
    if (form.is_registered) {
        const registration_num =
            form.registration_number == null
                ? ''
                : String(form.registration_number);
        if (registration_num.length !== 8) {
            form.setError(
                'registration_number',
                'Registration number must be exactly 8 characters long',
            );
            hasErrors = true;
        }
    }
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mt-4 flex justify-center px-4">
            <form
                ...
                class="fs-32 w-full max-w-4xl min-w-[min(100%,20rem)] space-y-8 rounded-xl border border-border bg-card p-8 shadow-sm"
            >
                ...
                <div class="space-y-2">
                    ...
                    <p
                        v-if="form.errors.is_registered"
